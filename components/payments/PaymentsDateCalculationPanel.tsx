...
export interface PaymentsDateCalculationPanelProps {
  label: React.ReactNode;
  selectedDate: string;
  minDate?: string;
  isLoading?: boolean;
  onCalculate: (date: string) => void;
  /** Например кнопка «Создать выплату» на вкладке управления */
  rightSlot?: React.ReactNode;
}

export default function PaymentsDateCalculationPanel({
  label,
  selectedDate,
  minDate,
  isLoading = false,
  onCalculate,
  rightSlot,
}: PaymentsDateCalculationPanelProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-col gap-3 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        {rightSlot ? <div className="flex flex-shrink-0 justify-end">{rightSlot}</div> : null}
      </div>
