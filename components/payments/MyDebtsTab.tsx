...
export default function MyDebtsTab({ myDebts, onShowCalculation }: MyDebtsTabProps) {
    ...
    return (
      <Card className="p-12 text-center">
        ...
        <p className="text-gray-600">
          У вас нет невыплаченных задолженностей
        </p>
      </Card>
    );
  }
          ...
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-4">
                <div className={`w-4 h-4 rounded-full ${
                  debt.isPaymentDue ? 'bg-red-400 animate-pulse' : 'bg-yellow-400'
                }`} />
                <div>
                  <h3 className="text-xl font-bold text-gray-900">{debt.workName}</h3>
                  <div className="flex items-center space-x-2 text-sm text-gray-500">
                    <UserIcon className="h-4 w-4" />
                    <span>Ответственный: {debt.responsibleUser.firstName || ''} {debt.responsibleUser.lastName || ''}</span>
                  </div>
                </div>
              </div>
            ...
            {debt.lastClosureDate && (
              <div className="flex items-center space-x-2 mb-4 text-sm text-gray-600">
                ...
                <span>Последнее закрытие: {new Date(debt.lastClosureDate).toLocaleDateString('ru-RU')}</span>
              </div>
            )}
                ...
                >
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="text-right">
                      <p className="text-lg font-bold text-green-600">{formatCurrency(duty.totalDebt)}</p>
                      <p className="text-xs text-gray-500">мне должны</p>
                    </div>
                    
                    <Button
                      onClick={() => onShowCalculation('current-user', debt.workId, duty.id)}
                      size="sm"
