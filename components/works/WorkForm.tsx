...
const WorkForm: React.FC<WorkFormProps> = ({
  ...
  const salaryValue = typeof formData.salary === 'string' ? parseFloat(formData.salary) || 0 : formData.salary || 0;
        ...
        {isFormValid && (
          <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-4 border-l-4 border-green-400">
            <div className="flex items-center mb-2">
              ...
              <h4 className="text-sm font-medium text-green-800">Предварительный просмотр</h4>
            </div>
            <div className="text-sm text-green-700">
              <p><span className="font-medium">Название:</span> {formData.name}</p>
              <p><span className="font-medium">Бюджет:</span> {formatCurrency(salaryValue)}</p>
              {formData.releaseDate && (
                <p><span className="font-medium">Дата выхода:</span> {new Date(formData.releaseDate).toLocaleDateString('ru-RU')}</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor="responsibleUserId" className="block text-sm font-medium text-gray-700 mb-2">
                    <svg className="w-4 h-4 inline mr-1 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <Select
                    ...
                    options={userOptions}
                  />
                  <p className="mt-1 text-xs text-gray-500">Пользователь, отвечающий за выполнение работы</p>
                </div>
              ...
              <div>
                ...
                {salaryValue > 0 && (
                  <div className="mt-3 p-3 bg-white rounded border border-primary-200">
                    ...
                    <div className="text-lg font-semibold text-primary-800">{formatCurrency(salaryValue)}</div>
                  </div>
                )}
              </div>
            </div>
