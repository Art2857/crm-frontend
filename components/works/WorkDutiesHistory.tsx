...
const WorkDutiesHistory: React.FC<WorkDutiesHistoryProps> = ({ 
  ...
  const validDistributions = useMemo(() => {
    ...
    return distributions.filter(dist => {
      ...
      if (showOnlyCurrentUser && currentUserId) {
        const hasUserDuties = dist.details.some(detail => 
          detail.user && detail.user.id === currentUserId
        );
        if (!hasUserDuties) {
          return false;
        }
  ...
  const getDifferences = (current: DistributionWithDetails, previous: DistributionWithDetails | null) => {
    if (!previous) return {};
    ...
    current.details.forEach(currentDetail => {
      ...
      const prevDetail = previous.details.find(d => 
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {canEdit && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => startEditingEffectiveDate(workHistoryItem.id, workHistoryItem.effectiveDate)}
        ...
        <div className="px-6 py-3 bg-gray-50">
          <div className="text-xs text-gray-500">
            <span className="font-medium">Дата создания изменения:</span> {formatDateForDisplay(workHistoryItem.updatedAt || workHistoryItem.createdAt, true)}
          </div>
        </div>
      </div>
    );
  ...
  const renderDistributionItem = (distribution: DistributionWithDetails, index: number, groupIndex: number) => {
    ...
    const filteredDetails = showOnlyCurrentUser && currentUserId 
      ...
      <div className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-600 font-medium">{distribution.workHistory.name}</p>
              </div>
            </div>
            <div className="text-right">
              <div className="text-sm text-gray-500">Зарплата</div>
              <div className="text-lg font-semibold text-gray-900">
                {Number(distribution.workHistory.salary || 0).toLocaleString()} тыс. ₽
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-emerald-50 to-green-50 border-b border-emerald-100">
          <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {canEdit && (
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => startEditingEffectiveDate(distribution.workHistory.id, distribution.workHistory.effectiveDate)}
            ...
            {/* Ответственный */}
            {distribution.workHistory.responsibleUserId && usersMap[distribution.workHistory.responsibleUserId] && (
              <div className="text-right">
                <div className="text-sm text-gray-500">Ответственный</div>
                <div className="text-sm font-medium text-gray-900">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredDetails.map((detail) => {
                ...
                const userName = user 
                  ...
                  : (detail.user?.email || (detail.user?.lastName && detail.user?.firstName) ? `${detail.user?.lastName || ''} ${detail.user?.firstName || ''}`.trim() : `ID: ${detail.user?.id || 'неизвестно'}`);
                ...
                const numericCalculatedValue = detail.calculatedValue ? parseFloat(detail.calculatedValue) : null;
                
                // Преобразуем workSalary в число для расчетов
                const numericSalary = typeof workSalary === 'string' ? parseFloat(workSalary) : workSalary;
        ...
        <div className="px-6 py-3 bg-gray-50">
          <div className="text-xs text-gray-500">
            <span className="font-medium">Дата создания изменения:</span> {formatDateForDisplay(distribution.createdAt, true)}
          </div>
        </div>
      </div>
    );
  ...
  // Компонент для отображения сворачиваемой группы
  const renderCollapsibleGroup = React.useCallback((collapsibleGroup: {
    groupKey: string;
    groupName: string;
    items: Array<{
      type: 'workHistory' | 'distribution';
            ...
            >
              <div className="flex items-center space-x-2">
                <span className="text-sm font-medium text-gray-700">
                  {isExpanded ? 'Скрыть предыдущие изменения' : `Показать предыдущие изменения (${hiddenItems.length})`}
                </span>
                <span className={`transform transition-transform duration-200 text-gray-500 ${isExpanded ? 'rotate-180' : 'rotate-0'}`}>
                  ▼
                </span>
              </div>
            </button>
                ...
                {hiddenItems.map((item, index) => (
                  <div key={`${item.type}-${item.createdAt}-hidden`} className="history-group">
                    {item.type === 'workHistory' ? (
                      ...
                      renderDistributionItem(item.data as DistributionWithDetails, index + 1, groupIndex)
                    )}
                  </div>
                ))}
              </div>
  ...
  }, [expandedGroups, toggleGroupExpansion, renderWorkHistoryItem, renderDistributionItem]);
        ...
        <div className="pb-4">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Полная история модификаций обязанностей в проекте</p>
            </div>
          </div>
          ...
          <div className="space-y-8">
            {groupedHistory.length > 0 ? (
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-emerald-600">
                          {group.items.length} {group.items.length === 1 ? 'изменение' : group.items.length < 5 ? 'изменения' : 'изменений'}
                        </p>
                      </div>
                    </div>
                  </div>
                      ...
                      group.items.map((item, index) => (
                        <div key={`${item.type}-${item.createdAt}`} className="mb-4">
                          {item.type === 'workHistory' ? (
                            ...
                            renderDistributionItem(item.data as DistributionWithDetails, index, groupIndex)
                          )}
                        </div>
                      ))
                    )}
              ...
              <div className="text-center py-12 bg-gray-50 rounded-lg">
                ...
                <p className="text-gray-400 text-sm mt-2">Изменения будут отображаться здесь после их создания</p>
              </div>
            )}
          </div>
        </div>
