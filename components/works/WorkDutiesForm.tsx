...
interface WorkDutiesFormProps {
  ...
  onSubmit: (duties: Array<{
    dutyId: string;
    userId: string;
    price: string | null;
    percentage: string | null;
  }>, effectiveDate?: string) => void;
  onCancel: () => void;
  isLoading?: boolean;
  workSalary?: string;
}
...
const WorkDutiesForm: React.FC<WorkDutiesFormProps> = ({
  ...
  const [effectiveDate, setEffectiveDate] = useState<string>(getCurrentDateISO());
  ...
  const getDefaultValuesForDuty = (dutyId: string) => {
    ...
    return {
      price: selectedDuty?.basePrice !== null && selectedDuty?.basePrice !== undefined 
        ? selectedDuty.basePrice.toString() 
        : '',
      percentage: selectedDuty?.basePercentage !== null && selectedDuty?.basePercentage !== undefined
  ...
  const salaryDistributionData = useMemo(() => {
    const numericSalary = typeof workSalary === 'string' ? Number(workSalary) : 0;
    ...
    dutyItems.forEach((item) => {
      ...
      const dutyName = duties.find(d => d.id === item.dutyId)?.name || 'Обязанность';
    ...
    dutyItems.forEach(item => {
      ...
      if (calculations.has(dutyId)) {
        ...
        if (existing.maxValue !== null && existing.total > existing.maxValue) {
          ...
          existing.percentComplete = existing.minValue === 0 ? 0 : (existing.total / existing.minValue) * 100;
          existing.isExceeded = false;
        } else if (existing.maxValue !== null) {
          const percentage = existing.minValue !== null 
            ? ((existing.total - existing.minValue) / (existing.maxValue - existing.minValue)) * 100
        ...
        if (maxValueNumeric !== null && totalAmount > maxValueNumeric) {
          ...
          const percentage = minValueNumeric !== null 
  ...
  const validateForm = () => {
    ...
    const invalidDuties = dutyItems.some(
      item => !item.dutyId || !item.userId || (!item.price && !item.percentage)
    );
    
    if (invalidDuties) {
      setValidationError('Для каждой обязанности необходимо указать обязанность, ответственного и стоимость или процент');
      return false;
    }
    
    // Проверка распределения (не должно превышать 100%)
    if (totalPercentage > 100) {
      setValidationError(`Общая сумма распределения (${totalPercentage.toFixed(2)}%) превышает 100%. Пожалуйста, скорректируйте значения.`);
      return false;
    }
  ...
  const getStatusColor = (status: 'ok' | 'warning' | 'error' | 'below-min') => {
    switch (status) {
      ...
      case 'below-min': return 'bg-blue-500';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const formattedDuties = dutyItems.map((item) => ({
        ...
        percentage: item.percentage && item.percentage.trim() !== '' ? item.percentage : null,
      }));
  ...
  const renderPieChart = () => {
    ...
    return (
      <div className="flex flex-col items-center mt-4">
        <h4 className="text-md font-medium text-gray-700 mb-2">Предварительный просмотр распределения</h4>
          ...
          {salaryDistributionData.map((item, index) => (
            <div key={index} className="flex items-center">
              ...
              <span className="text-sm font-medium ml-1">
                {formatCurrency(item.value)} {toNumber(workSalary) > 0 ? `(${Math.round((item.value / toNumber(workSalary)) * 100)}%)` : ''}
              </span>
            </div>
          ))}
        </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="space-y-6">
          <div className="border-b pb-4">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Редактирование обязанностей</h3>
            
            {dutyItems.length === 0 ? (
              <p className="text-gray-500 italic mb-4">Нет назначенных обязанностей</p>
            ) : (
              dutyItems.map((item, index) => (
                <div key={index} className="flex flex-col md:flex-row gap-3 mb-6 md:mb-4 items-end border-b pb-4 md:pb-2 last:border-b-0">
                    ...
                    >
                      {users.map((user) => (
                        <option key={user.id} value={user.id}>
                          {`${user.lastName || ''} ${user.firstName || ''}`.trim() || user.email || 'Пользователь'}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="flex-1 md:w-1/6">
                    ...
                    {(() => {
                      ...
                      const isBelow = minValueNumeric !== null && total < minValueNumeric;
                      const isAbove = maxValueNumeric !== null && total > maxValueNumeric;
                      
                      // Проверяем, активна ли обязанность (заполнены ли поля)
                      const isActive = (item.price && item.price !== '') || (item.percentage && item.percentage !== '');
                      ...
                      const adjustDutyAmount = () => {
                        ...
                        if (isBelow && minValueNumeric !== null) {
                          handleChange(index, 'price', minValueNumeric.toString());
                        } 
                        // Если сумма выше максимального значения, уменьшаем до максимума
                        else if (isAbove && maxValueNumeric !== null) {
                          handleChange(index, 'price', maxValueNumeric.toString());
                        }
            ...
            {dutyItems.some(item => (item.percentage && item.percentage !== '') || (item.price && item.price !== '')) && (
                ...
                {totalPercentage > 100 && (
                  <div className="w-full bg-red-200 rounded-full h-2.5 mt-1 overflow-hidden">
                    <div 
                      ...
                      style={{ width: `${Math.min((totalPercentage - 100) * 5, 100)}%` }}
                    ></div>
                  </div>
                )}
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {numericSalary > 0 && (
                <Button
                  ...
                  tabIndex={0}
                >
                  {showSalaryPreview ? 'Скрыть предпросмотр' : 'Показать предпросмотр'}
                </Button>
              )}
