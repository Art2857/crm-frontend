...
export default function WorkDocuments({ workId }: Props) {
  ...
  const deleteConfirm = useConfirmation<string>(async (id: string) => {
    try {
      ...
      notification.showSuccess('Файл удалён');
  ...
  const fetchDocs = async () => {
    try {
      setLoading(true);
      const list = await documentsService.listForWork(workId);
      setDocuments(list);
    } catch (err: any) {
      // eslint-disable-next-line no-console
      console.error('Failed to load work documents', err);
      const msg = err?.originalData?.message || err?.message || 'Не удалось получить список документов';
      notification.showError(msg, 8000);
    } finally {
      setLoading(false);
    }
  ...
  const onSaveAdd = async () => {
    ...
    try {
      setSaving(true);
      await documentsService.uploadForWork({ workId, name: addName.trim(), file: addFile });
      setIsAddOpen(false);
      notification.showSuccess('Файл успешно загружен');
      await fetchDocs();
    } catch (err: any) {
      // eslint-disable-next-line no-console
  ...
  const onDownload = async () => {
    ...
    try {
      ...
      const msg = err?.originalData?.message || err?.message || 'Не удалось получить ссылку на скачивание';
      notification.showError(msg, 8000);
    }
  };
  ...
  const onDownloadAll = async () => {
    try {
      const blob = await documentsService.downloadWorkZip(workId);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `work-${workId}-documents.zip`;
      document.body.appendChild(a);
      a.click();
      a.remove();
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Поддерживаемые форматы: PNG, JPEG, JPG, GIF, WEBP, PDF, DOC, DOCX
              </p>
            </div>
          </div>
        ...
        <div className="relative w-[380px] max-w-[90vw] p-4">
          ...
          <div className="text-sm mb-4">
            ...
            {selectedDoc?.originalName && (
                <div className="mt-2">
                  <div className="text-gray-500">Исходное имя файла</div>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
                ...
                className="px-2 py-2 min-h-[32px] min-w-[32px] !bg-red-50 border !border-red-300 hover:bg-red-700 !text-gray-700 hover:!text-white hover:!bg-red-500"
            ...
            <Button
                ...
                variant="outline"
                onClick={onPreview}
                className="px-2 py-2 min-h-[32px] min-w-[32px] !bg-green-50 !border-green-300 bg-primary-100 hover:!bg-green-500 hover:!text-white"
            ...
            <Button
                ...
                variant="outline"
                onClick={onDownload}
                className="px-2 py-2 min-h-[32px] min-w-[32px] !bg-blue-50 !border-blue-300 bg-primary-100 hover:!bg-blue-500 hover:!text-white"
