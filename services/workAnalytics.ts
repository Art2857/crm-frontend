...
export const workAnalyticsService = {
   ...
   */
  getAnalytics: async (archived = false): Promise<WorkAnalyticsResponse> => {
    try {
      const endpoint = archived ? WORKS_ENDPOINTS.analyticsArchived : WORKS_ENDPOINTS.analytics;
   ...
   */
  getAnalyticsForUser: async (userId: string): Promise<WorkAnalytics[]> => {
    try {
      const response = await privateApi.get<WorkAnalytics[]>(WORKS_ENDPOINTS.analyticsUser(userId));
      return response.data;
    } catch (error) {
