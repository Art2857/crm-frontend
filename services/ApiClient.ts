...
export class ApiClient {
  ...
  constructor(options: ApiClientOptions = {}) {
    ...
    if (ignoreSSL) {
      createAxiosDefaults.httpsAgent = new https.Agent({ rejectUnauthorized: false });
    }
    ...
    if (error.response) {
      ...
      if (status === 400) {
        if (typeof data === 'object' && data !== null) {
          ...
          if ('validationErrors' in data && typeof data.validationErrors === 'object') {
        ...
        errorMessage = typeof data === 'string' ? data : 'Ошибка валидации данных';
      } else if (status === 403) {
