...
/**
 * Глобальный объект для отладки (совместимость со старым API)
 */
if (typeof window !== 'undefined') {
  (window as any).__debugExchangeRates = {
    // Основные методы
    async getLatestRate(currencyCode: string = 'USD') {
      return exchangeRateFacade.getLatestRate(currencyCode);
    },
    
    async forceUpdate(currencyCode: string = 'USD') {
      return exchangeRateFacade.forceUpdate(currencyCode);
    },
    
    async smartUpdate(currencyCode: string = 'USD') {
      return exchangeRateFacade.smartUpdate(currencyCode);
    },
    
    async getCacheInfo() {
      return exchangeRateFacade.getCacheInfo();
    },
    
    async clearCache() {
      return exchangeRateFacade.clearCache();
    },
    ...
    testWorkingDays() {
      return exchangeRateFacade.testWorkingDays();
    },
    
    getWorkingDaysInfo(date?: string) {
      return exchangeRateFacade.getWorkingDaysDebugInfo(date);
    },
    ...
    // Информация
    getSupportedCurrencies() {
      return exchangeRateFacade.getSupportedCurrencies();
    },
  ...
  console.log('🛫 Отладочный интерфейс доступен через __debugExchangeRates');
}
