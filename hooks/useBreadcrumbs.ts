...
export const useBreadcrumbs = (customBreadcrumbs?: Breadcrumb[]) => {
  ...
  useEffect(() => {
    if (customBreadcrumbs) {
      ...
      if (pathname === '/dashboard') {
      ...
      } else if (pathname.startsWith('/works/') && pathname !== '/works/create') {
        // Получаем ID работы из пути
        const workId = pathname.replace('/works/', '');
        dynamicBreadcrumbs = breadcrumbUtils.createWorkDetailBreadcrumbs(undefined, workId);
        ...
        dynamicBreadcrumbs = breadcrumbUtils.createDistributionsAdminBreadcrumbs();
      } else {
...
const findMatchingPath = (currentPath: string): string | undefined => {
  ...
  const paths = Object.keys(breadcrumbConfig).sort((a, b) => b.length - a.length);
